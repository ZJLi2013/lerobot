...
docker run --rm  -it --gpus all \
  ...
  bash -c 'pip install -e "." -q \
           ...
           && python quick_starts/viz_dataset_so101.py \
                --mode web \
                --episode-index 0 \
                --web-port 9090 \
                --grpc-port 9876'
...
```bash
# Docker 方式
bash ~/github/lerobot/quick_starts/run_viz_docker.sh save 0
...
```bash
# Docker 方式
bash ~/github/lerobot/quick_starts/run_viz_docker.sh stats 0
...
### Q1：浏览器打开 `http://IP:9090` 出现 "无法连接"

**原因**：服务器防火墙封锁了 9090 端口。  
**解决**：使用 SSH 端口转发（见 Step 3），访问 `http://localhost:9090`。

### Q2：Docker 内运行但端口无法访问

**检查命令**：
```bash
# 确认 docker run 带了 -p 参数
docker ps --format "{{.Ports}}"
# 应看到: 0.0.0.0:9090->9090/tcp, 0.0.0.0:9876->9876/tcp
```

**原因**：使用 `run_viz_docker.sh` 而不是手动 docker run，手动 docker run 必须加 `-p 9090:9090 -p 9876:9876`。
...
# 或通过 lerobot 安装（自动处理依赖）
pip install "lerobot[rerun]"
```

### Q4：torchcodec 报错（ABI 兼容问题）

与训练测试一样，安装完后卸载 torchcodec：
```bash
pip uninstall torchcodec -y
```
lerobot 会自动降级到 `pyav` 后端。

### Q5：stats 模式生成的图表是空的

**原因**：数据集下载失败或 episode 索引越界。  
**检查**：
```bash
python -c "
from lerobot.datasets.lerobot_dataset import LeRobotDatasetMetadata
meta = LeRobotDatasetMetadata('lerobot/svla_so101_pickplace')
print('总 episodes:', meta.total_episodes)  # 应为 50
"
```

### Q6：Web Viewer 加载很慢

**原因**：视频帧数据较大，通过网络传输需要时间。  
**建议**：
- 优先使用 `--mode save` 生成 .rrd 文件，本地查看速度更快
- 或减小 `--batch-size`（默认 32）避免内存压力

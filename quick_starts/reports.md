...
```powershell
cd C:\Users\zhengjli\Documents\github\lerobot
git add quick_starts/
git commit -m "Add quick_starts test scripts"
# 若远端 fork 有新提交，先 rebase
git pull --rebase origin main
git push origin main
```

注意：本项目 origin 指向 `ZJLi2013/lerobot`（个人 fork），不是 `huggingface/lerobot`。
...
cd ~/github
git clone https://github.com/ZJLi2013/lerobot.git
```

后续更新只需：
```bash
ssh david@10.161.176.110 "cd ~/github/lerobot && git pull --ff-only"
```
...
docker run --rm --gpus all \
  ...
  bash -c 'pip install -e ".[pusht]" -q \
           ...
           && python quick_starts/test_diffusion_pusht.py'
```
...
docker run --rm --gpus all \
  ...
  bash -c 'pip install -e . -q \
           && pip uninstall torchcodec -y -q \
           && python quick_starts/test_act_so101.py'
```
...
docker run --rm --gpus all \
  ...
  bash -c 'pip install -e ".[smolvla]" -q \
           && pip uninstall torchcodec -y -q \
           && python quick_starts/test_smolvla_so101.py'
```

> 或直接运行仓库内的封装脚本：`bash quick_starts/run_smolvla_docker.sh`
